# Documentation Hub

Welcome to the central documentation hub for Equipose. This documentation is organized according to the [Diátaxis](https://diataxis.fr/) framework.

## Tutorials
*Learning-oriented resources that take you by the hand through a series of steps to complete a project.*

(No tutorials currently available.)

## How-To Guides
*Problem-oriented guides that provide steps to achieve a specific goal.*

- [Release Checklist for Regulated and Statistical-Confidence Use](how-to/RELEASE_CHECKLIST.md)
- [Manual Accessibility Audit Protocol](how-to/MANUAL_A11Y_AUDIT_PROTOCOL.md)
- [Verification of Clinical Export Audit Trail Hash (21 CFR Part 11)](how-to/VERIFY_AUDIT_HASH.md)

## Reference
*Information-oriented technical descriptions and reference materials.*

- [Performance Budgets](reference/PERFORMANCE_BUDGETS.md)
- [Architecture Reference - Equipose](reference/ARCHITECTURE_REFERENCE.md)
- [Developer Command Reference](reference/COMMANDS.md)

## Explanation
*Understanding-oriented discussions that clarify concepts and background.*

- [Architecture Concepts - Equipose](explanation/ARCHITECTURE_CONCEPTS.md)
- [Seed Compression & Regulatory Compliance Explainer](explanation/SEED_COMPLIANCE_EXPLAINER.md)
- [Standardized Persona Framework and Validation Strategy](explanation/PERSONA_STRATEGY.md)
- [SAS & Stata Cross-Environment Exception Report](explanation/SAS_Stata_Exception_Report.md)
- [ADR Template](explanation/adr/template.md)
- [ADR 0001 — SAS Script Static Validation Strategy](explanation/adr/0001-sas-static-validation-strategy.md)
