...
async function verifyPayload(page: Page, expectedStrategy: string, checkFn?: (config: any) => void) {
  ...
  // Go back to step 5
  await page.getByRole('button', { name: /^Previous$/i }).evaluate(b => (b as HTMLElement).click());
...
test.describe('Enrollment Cap Strategy Switching', () => {
  ...
  test('should transition from MANUAL_MATRIX to PROPORTIONAL and compute matrix', async ({ page }) => {
    ...
    await propButton.evaluate(b => (b as HTMLElement).click());
    ...
    await computeButton.evaluate(b => (b as HTMLElement).click());
  ...
  test('should transition from PROPORTIONAL to MANUAL_MATRIX when editing a computed cap', async ({ page }) => {
    ...
    await page.getByRole('radio', { name: /Proportional/i }).evaluate(b => (b as HTMLElement).click());
    ...
    await page.locator('input[id="age-pct->=65"]').blur();
    // tab
    await page.getByRole('button', { name: /Compute Matrix/i }).evaluate(b => (b as HTMLElement).click());
  ...
  test('should transition from MANUAL_MATRIX to MARGINAL_ONLY and back', async ({ page }) => {
    ...
    await marginalButton.evaluate(b => (b as HTMLElement).click());
    ...
    await manualBtn.evaluate(b => (b as HTMLElement).click());
