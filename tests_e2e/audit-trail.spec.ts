...
async function downloadCodeFile(
  ...
  await nextBtn.dispatchEvent('click'); // → Arms

  await nextBtn.dispatchEvent('click'); // → Sites
  ...
  await nextBtn.dispatchEvent('click'); // → Blocks

  await page.locator('#blockSizesStr').fill('4');
  await nextBtn.dispatchEvent('click'); // → Strata
  await nextBtn.dispatchEvent('click'); // → Review
  ...
  await page.getByRole('menuitem', { name: new RegExp(language, 'i') }).dispatchEvent('click');
