...
test.describe('Determinism Test Suite', () => {
  test('generates identical Audit Hash for the same seed across Chromium, WebKit, and Firefox', async ({ page }) => {
    ...
    await page.locator("button:has-text('Next'):visible").first().evaluate(b => (b as HTMLElement).click());

    await page.getByRole('radio', { name: 'Minimization' }).evaluate(b => (b as HTMLElement).click());
    ...
    for (let i = 0; i < 2; i++) {
      await page.getByRole('button', { name: /Increase ratio for Low Dose/i }).evaluate(b => (b as HTMLElement).click());
    }
    for (let i = 0; i < 6; i++) {
      await page.getByRole('button', { name: /Increase ratio for Placebo/i }).evaluate(b => (b as HTMLElement).click());
    }
    await page.locator("button:has-text('Next'):visible").first().evaluate(b => (b as HTMLElement).click());
    ...
    await page.getByRole('radio', { name: 'Marginal Only' }).evaluate(b => (b as HTMLElement).click());
