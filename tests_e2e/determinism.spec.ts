...
test.describe('Determinism Test Suite', () => {
  test('generates identical Audit Hash for the same seed across Chromium, WebKit, and Firefox', async ({ page }) => {
    ...
    for (let i = 0; i < 2; i++) {
      await page.getByRole('button', { name: /Increase ratio for Low Dose/i }).dispatchEvent('click');
    }
    for (let i = 0; i < 6; i++) {
      await page.getByRole('button', { name: /Increase ratio for Placebo/i }).dispatchEvent('click');
    }
    await page.locator("button:has-text('Next'):visible").first().dispatchEvent('click');
    ...
    await page.getByRole('radio', { name: 'Marginal Only' }).dispatchEvent('click');
