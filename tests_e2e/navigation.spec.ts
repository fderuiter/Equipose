...
test.describe('Application Navigation', () => {
  ...
  test('should navigate to the generator page via the "New Study" link', async ({ page }) => {
    await page.goto('http://localhost:4200');
    await page.getByRole('link', { name: /New Study/i }).evaluate(b => (b as HTMLElement).click());
  ...
  test('should navigate to the About page via the header nav link', async ({ page }) => {
    ...
    if (isMobile) {
      ...
      if (!(await menu.isVisible())) {
        await page.getByRole('button', { name: 'Toggle navigation menu' }).evaluate(b => (b as HTMLElement).click());
        await expect(menu).toBeVisible();
      }
      await menu.getByRole('link', { name: /About/i }).evaluate(b => (b as HTMLElement).click());
    } else {
      await page.locator('header').getByRole('link', { name: /About/i }).evaluate(b => (b as HTMLElement).click());
    }
  ...
  test('should navigate to the Generator page via the header nav link', async ({ page }) => {
    ...
    if (isMobile) {
      ...
      await menu.getByRole('link', { name: 'Generator', exact: true }).evaluate(b => (b as HTMLElement).click());
    } else {
      await page.locator('header').getByRole('link', { name: 'Generator', exact: true }).evaluate(b => (b as HTMLElement).click());
    }
  ...
  test('should navigate back to the landing page via the logo link', async ({ page }) => {
    ...
    await page.getByRole('link', { name: /Equipose/ }).first().evaluate(b => (b as HTMLElement).click());
