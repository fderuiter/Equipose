      ...
      <div class="space-y-6">
        ...
        <div class="flex flex-wrap items-center gap-4 text-xs text-gray-500 dark:text-slate-400">
          ...
          <span class="inline-flex items-center gap-1.5">
            ...
            Expected deviation (incomplete block)
          </span>
          <span class="inline-flex items-center gap-1.5">
...
export class BalanceVerificationComponent {
  ...
  /**
   * Given a flat array of schema rows, produces an array of ArmBalance objects
   * (one per treatment arm) with actual, target, variance, and status.
   */
  private buildArmBalances(rows: GeneratedSchema[]): ArmBalance[] {
    ...
    return arms.map(arm => {
      ...
      if (absVariance === 0) {
        ...
        status = 1; // expected incomplete block
      } else {
        status = 2; // critical
      }
  ...
  readonly stratumRows = computed<BalanceRow[]>(() => {
    ...
    // Group by the compound stratum key: "site|factorA=levelA|factorB=levelB"
    const grouped = new Map<string, GeneratedSchema[]>();
    for (const row of schema) {
