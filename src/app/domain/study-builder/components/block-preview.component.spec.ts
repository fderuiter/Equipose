...
const mockGetArmColor = (idx: number) => MOCK_COLORS[idx % MOCK_COLORS.length];
...
describe('buildPreviews()', () => {
  it('should return empty array when no arms are provided', () => {
    expect(buildPreviews([], [4], mockGetArmColor)).toEqual([]);
  });

  it('should return empty array when no block sizes are provided', () => {
    expect(buildPreviews(TWO_ARMS, [], mockGetArmColor)).toEqual([]);
  });

  it('should compute a valid preview for a 1:1 ratio with block size 2', () => {
    const previews = buildPreviews(TWO_ARMS, [2], mockGetArmColor);
  ...
  it('should mark block size as invalid when not divisible by totalRatio', () => {
    ...
    const previews = buildPreviews(THREE_ONE_ARMS, [4], mockGetArmColor);
  ...
  it('should produce previews for multiple block sizes', () => {
    const previews = buildPreviews(TWO_ARMS, [2, 4, 6], mockGetArmColor);
  ...
  it('should not include zero or negative block sizes in previews', () => {
    const previews = buildPreviews([{ id: 'A', name: 'Active', ratio: 1 }], [0, -1, 2], mockGetArmColor);
  ...
  it('should have only valid slots when block size is valid', () => {
    const previews = buildPreviews(TWO_ARMS, [4], mockGetArmColor);
  ...
  it('should correctly use arm colors from the mock palette', () => {
    const previews = buildPreviews(TWO_ARMS, [2], mockGetArmColor);

    expect(previews[0].slots[0].bgClass).toBe(MOCK_COLORS[0].bgClass);
    expect(previews[0].slots[1].bgClass).toBe(MOCK_COLORS[1].bgClass);
  });
});
...
describe('BlockPreviewComponent (Angular)', () => {
  ...
  it('armColor() should cycle when index exceeds palette length', () => {
    ...
    expect(comp.armColor(8)).toBe(comp.armColor(0));
  });
